...
function subtractCurrentDateTime(amount: number, unit: unitOfTime.DurationConstructor) {
    return currentTimeInUTC().subtract(amount, unit);
}

function subtractCurrentDateTimeWithTimeZone(amount: number, unit: unitOfTime.DurationConstructor) {
    return subtractCurrentDateTime(amount, unit);
}

export function subtractCurrentDateTimeWithTimeZoneInDateTimeFormat(amount: number, unit: unitOfTime.DurationConstructor, timezone: string) {
    return convertTimeToTimeZone
    (subtractCurrentDateTimeWithTimeZone(amount, unit),
        timezone).format(DATE_TIME_FORMAT)
}
